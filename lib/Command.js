	...
	constructor(name, fn, desc = '', args = [], flags = [], async = false) {
		...
		for (var i = 0; i < args.length; i++) {
			...
			if (!this.args[args[i].name].required && typeof args[i].default !== 'undefined') {
				if (typeof args[i].default !== args[i].type) {
					throw new Error('Error creating command ' + name +
						': argument default value for argument ' + args[i].name +' does not' +
			...
			(!this.args[args[i].name].required && typeof args[i].default === 'undefined') {
				throw new Error('Error creating command ' + name +
					': argument ' + args[i].name +' is not required, but no default value' +
		...
		for (i = 0; i < flags.length; i++) {
			...
			if (
				typeof flags[i].default !== 'undefined' &&
				typeof flags[i].default !== flags[i].type
			) {
				throw new Error('Error creating command ' + name +
