	...
	constructor(options) {
		...
		for (let i = 0; i < options.args.length; i++) {
			if (typeof options.args[i].name !== "string")
				throw new Error("Error creating command " + options.name +
			...
			if (
				options.args[i].type !== "string"  &&
				options.args[i].type !== "number"
			) {
				throw new Error("Error creating command " + options.name +
					": argument types can only be string or number. Please refer to the" +
					" documentation.");
			}
			 ...
			 * @since 1.0.0
			 */
			let validations = options.args[i].validations || [];
			if (!Array.isArray(validations)) {
				throw new Error("Error creating command " + options.name + ": validations must be "
					+ "in an array.");
			}

			for (let j = 0; j < validations.length; j++) {
				if (
					typeof validations[j].errorMessage !== "string" ||
					typeof validations[j].validate     !== "function"
				) {
					throw new Error("Error creating command " + options.name + ": a provided" +
						" validation is missing one of their parameters. Please refer to the" +
						" documentation.");
				}
				...
				if (typeof validations[j].validate(testVal) !== "boolean") {
					throw new Error("Error creating command " + options.name + ": one of your" +
						" validations was tested, but it didn't return a boolean value.");
				}
			}

			this.args[options.args[i].name] = {
				required: (typeof options.args[i].required === "boolean")
					? options.args[i].required : false,
				desc: (typeof options.args[i].desc === "undefined")
					? undefined : options.args[i].desc,
			...
			if (
				!this.args[options.args[i].name].required &&
				typeof options.args[i].default !== "undefined"
			) {
				if (typeof options.args[i].default !== options.args[i].type) {
					throw new Error("Error creating command " + options.name +
						": argument default value for argument " + options.args[i].name +
						" does not match its data type. Please refer to the documentation.");
				}
			...
			(
				...
				throw new Error("Error creating command " + options.name +
					": argument " + options.args[i].name + " is not required, but no default" +
					" value was provided. Please refer to the documentation.");
			}
		}
		...
		for (let i = 0; i < options.flags.length; i++) {
			if (typeof options.flags[i].name !== "string")
				throw new Error("Error creating command " + options.name +
		                   ": unnamed flag. Please refer to the documentation.");

			if (typeof options.flags[i].type !== "string")
				throw new Error("Error creating command " + options.name +
		                   ": unspecified flag type. Please refer to the documentation.");

			if (
				options.flags[i].type !== "boolean" &&
				options.flags[i].type !== "string"  &&
				options.flags[i].type !== "number"
			) {
				throw new Error("Error creating command " + options.name +
					": flag types can only be boolean, string or number. Please refer to the" +
					" documentation.");
			}

			if (typeof options.flags[i].alias === "string" && options.flags[i].alias.length !== 1)
				throw new Error("Error creating command " + options.name +
		                   ": aliases can only be one character long.");

			if (typeof options.flags[i].default === "undefined") {
				throw new Error("Error creating command " + options.name +
					": flag " + options.flags[i].name + " does not have a default value. Please" +
					" refer to the documentation.");
			}

			if (
				typeof options.flags[i].default !== "undefined" &&
				typeof options.flags[i].default !== options.flags[i].type
			) {
				throw new Error("Error creating command " + options.name +
					": flag default value for flag " + options.flags[i].name + " does not" +
					" match its data type. Please refer to the documentation.");
			}

			let validations = options.flags[i].validations || [];
			if (!Array.isArray(validations)) {
				throw new Error("Error creating command " + options.name + ": validations must" +
					" be in an array.");
			}
			...
			this.flags[options.flags[i].name] = {
				...
				default: (typeof options.flags[i].default === "undefined")
					? null : options.flags[i].default,
